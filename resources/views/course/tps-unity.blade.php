                    ...
                    @if ($tp->scoreUser(Auth::user()->id) == null || @$tp->scoreUser(Auth::user()->id)->nota == 'desaprobado')
                        @if (@$tp->scoreUser(Auth::user()->id)->nota == 'desaprobado')
                            <div class="alert alert-danger">
                                <strong>Desaprobado con {{ $tp->scoreUser(Auth::user()->id)->nota_numerica }}&nbsp;<i
                                        class="fas fa-exclamation-triangle"></i></strong>
                            </div>
                        @endif
                        ...
                        <div class="input-group justify-center">
                            ...
                            <div class="custom-file">
                                ...
                                <form class="form-control"
                                    action="{{ route('Tpresuelto.store', ['id' => Auth::user()->id, 'tp_id' => $tp->id, 'unity_id' => $unity->id]) }}"
                                    enctype="multipart/form-data" method="POST">
                                    @csrf
                                    <div class="mb-3">
                                        <label for="formFile" class="form-label">Selecciona tu archivo</label>
                                        <input class="form-control p-1 bg-red-200" type="file" name="tp" id="formFile">
                                    </div>
    ...
    <div class="col-md-6 p-3  tarjeta-unidad">
        <div class="card">
            ...
            <div class="card-body">
                ...
                @if ($tp->score != null)
                    @if ($tp->score->nota >= 7)
                        <div class="bg-success rounded text-center text-white p-2">Trabajo aprobado con <strong>
                                {{ $tp->score->nota }} </strong></div>

                    @elseif(strcmp(strtotime(date_format($tp->score->updated_at, 'Y-m-d')), strtotime(date('Y-m-d'))) !== 0)

                        <a href="{{ route('TpVf.showw', ['id' => $tp->id]) }}" class="btn btn-primary w-100">Intentar
                            Nuevamente</a>
                    @else

                        <div class="bg-danger rounded text-center p-2 text-white">Trabajo desaprobado, podra intentarlo
                            nuevamente mañana</div>
                    @endif
