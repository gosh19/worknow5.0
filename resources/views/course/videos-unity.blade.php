...
@endphp
    @foreach ($unity->videosYT as $i => $video)
        <div class="card tarjeta-video">
            ...
            <div id="collapse-{{$video->id}}" class="collapse p-3 caja-video" aria-labelledby="heading{{$video->id}}" data-parent="#accordionVideosYT">
                <h1 class="text-4xl">{{$video->titulo}}</h1>

                <h5 class="text-2x1 mb-3">{{$video->subtitulo}}</h5>
                <div class="row justify-content-center">
                    <iframe width="560" height="315" src={{$video->html}} frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                </div>
            </div>
        </div>
    @endforeach
    ...
    @endphp
        <div class="card tarjeta-video">
            <div class="card-header " id="heading{{($video->id+15)}}">
                <h2 class="mb-0">
                <button class="btn btn-link btn-block text-left text-white font-weight-bold" 
                        type="button" 
                        data-toggle="collapse" 
                        data-target="#collapse-{{($video->id+15)}}" 
                        aria-expanded="true" 
            ...
            <div id="collapse-{{($video->id+15)}}" class="collapse p-3 caja-video" aria-labelledby="heading{{($video->id+15)}}" data-parent="#accordionVideosYT">
