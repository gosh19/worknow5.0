    ...
    <tbody>
      @for($i = 0; $i < count($scores); $i++)
          @if (!$scores[$i]['scores']->isEmpty())
              ...
              <td><a href="{{route ('Tpresuelto.download', ['url' => $scores[$i]['url']])}}"> {{$scores[$i]['url']}}</a></td>
              @if($scores[$i]['scores'][$x]['nota'] == 'aprobado')
              <td><div class="alert alert-success"><strong>{{$scores[$i]['scores'][$x]['nota']}}</strong></div></td>
              @else
              <td><div class="alert alert-danger">{{$scores[$i]['scores'][$x]['nota']}}</div></td>
              @endif
              </tr>
              @endfor
